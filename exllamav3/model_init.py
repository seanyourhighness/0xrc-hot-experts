...
def add_args(
    ...
    parser.add_argument("-mcs", "--moe_cpu_split", type = int, help = "Experimental: per-layer expert split — run the TAIL N routed experts of every eligible block-sparse MoE layer on the CPU, overlapping the CPU GEMMs with each layer's own GPU expert compute. Static placement is the safe default; profile-seeded adaptive placement requires EXL3_MOE_CPU_SPLIT_STATS plus explicit SWAP and SWAP_SEED opt-in. Mutually exclusive with --moe_cpu_offload. Layer-split mode only; requires mul1-codebook experts", default = 0)
