...
A stats profile permutes router IDs into hot-to-cold order at load. Dynamic swap decisions are
therefore in router space, whereas checkpoint tensor keys remain in original-expert space. This
small no-CUDA test proves both sides of an in-place swap use translated checkpoint expert IDs.
"""
...
def test_seeded_swap_translates_router_ids_to_checkpoint_ids():
    # Router IDs [0, 1, 2, 3] refer to original checkpoint experts [2, 0, 3, 1].
    perm = [2, 0, 3, 1]
    ...
    for index in range(4):
        for prefix, base in (("u", 100), ("d", 200)):
            ...
            tensors[f"{prefix}{index}.suh"] = torch.full(
                (1,), base + index, dtype = torch.float16
            )
            tensors[f"{prefix}{index}.svh"] = torch.full(
                (1,), base + index, dtype = torch.float16
            )
    ...
    # Promote router id 2 (checkpoint expert 3) into the GPU slot serving router id 0
    # (checkpoint expert 2). The latter must be installed in the vacated worker slot.
